...
## ⚠️ 最高优先级：精确相关性过滤
搜索数据包含大量不相关结果。你**只分析跟本产品直接相关**的内容：
- 产品是"磁性钥匙收纳盒(magnetic key holder for car)" → 只分析物理钥匙收纳装置的专利
- 搜索返回了"encryption key(加密密钥)"、"cloud key management(云密钥)"、"digital access control(数字门禁)" → **全部忽略**
- 同理："hat clip(帽夹)" ≠ "paper clip(回形针)"；"drain cover(地漏)" ≠ "drain valve(排水阀)"
- **不确定时，宁可报告「未发现相关专利」，绝不强行关联无关专利**

## 审查框架
按五层递进：产品特征 → 版权 → 商标 → 设计专利 → 发明专利 → 市场验证 → 最终判断
- 设计专利：比对产品造型与专利附图的视觉相似性
- 发明专利：制作 Claim Chart，逐项比对权利要求要素
- TRO：发现执行历史 → 标记 🔴TRO → 风险自动升一级
- Keith 律所代理 → 风险至少 🟡
- 决策矩阵：任何一项🔴→❌No-Go；任何🟡→⚠️需修改；全🟢→✅Go
